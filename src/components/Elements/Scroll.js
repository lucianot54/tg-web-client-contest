...
export default function Scroll(parent, opts) {
  ...
  const scrollContainer = (offset, isSet) => {
    ...
    if(refs.container.scrollTop > 0 && opts.onHasScroll) opts.onHasScroll();
    else if(opts.onHasNoScroll) opts.onHasNoScroll();
  ...
  const updateHandlerHeight = (domHeight, containerHeight) => {
    ...
    if(newHandlerHeight != prevHandlerHeight) {
      ...
      if(newHandlerHeight >= (domHeight || dom.offsetHeight) - 17) {
        hideHandler();
        refs.handler.style.display = 'none';
      }
      else {
        scrollContainer(0);
        refs.handler.style.display = null;
      }
    }
  };
